...
Audio Tag 与 SWF 文件格式中的 DefineSound Tag 类似。它们的 Payload 数据除了额外的`Nellymoser 8-kHz`格式外是相同的，该格式在 SWF 格式中不被允许。关于 SWF 文件格式的介绍，请看[SWF(File Format Sepecification)](www.adobe.com/go/swf_file_formt).  
