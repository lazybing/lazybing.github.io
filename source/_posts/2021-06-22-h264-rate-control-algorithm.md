...
通常情况下，CRF 的值与 QP 都是严格一对一的，这里其实是并不准确的。比如当前帧画面复杂，人眼对其并不敏感，此时可以选择更大的 QP。基于此，优化思路是：根据当前帧的复杂度，优化 CRF 与 QP 之间的映射关系。这个在纯 CRF 下可以节省 3% 的码率。基本思想就是让更多的码率用于人眼敏感的平坦区域。这就实现了**帧级别的内容自适应**。
